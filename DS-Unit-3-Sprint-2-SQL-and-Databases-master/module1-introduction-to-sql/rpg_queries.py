...
avg_items = """
    SELECT AVG (COUNTER)
    FROM(
    SELECT character_id, COUNT(item_id) AS COUNTER
    FROM charactercreator_character_inventory
    GROUP BY character_id
    );
    """
...
if __name__ == "__main__":
    ...
    print("Number of weapons per character: ", execute_query(curs,Weapon_count))
    print("Average of items per character: ", execute_query(curs,avg_items))
    print("Average of weapons per character: ", execute_query(curs,avg_weaopns))
